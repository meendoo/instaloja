...
import { TimelineMax, TweenMax, Power4, Power2 } from 'gsap/all';
import Button from '../button';
require('gsap/ScrollToPlugin');
// import { ScrollToHash } from '../utils/scrollToHash/scrollToHash';
...
export default class Hero extends React.Component {
      ...
      titleTl
        ...
        .add(
          TweenMax.fromTo(
            article,
            0.3,
            { y: -45, opacity: 0, ease: Power4.easeIn },
            { y: 0, opacity: 1 }
          ),
          `article${i}`
        )
      ...
      heroTl
        ...
        .add(
          TweenMax.fromTo(
            ...
            { transform: 'scale(1.1)', ease: Power4.easeOut, opacity: 1 }
          )
        )
  ...
  render() {
    const images = this.props.data.edges.map((edges, i) => {
      return (
        <div
          ...
          style={{
            backgroundImage: `url('${
              edges.node.childImageSharp.fluid.originalImg
            }')`,
          }}
        />
      );
    ...
    const nav = this.props.data.edges.map((edges, i) => {
      return (
        <div key={'nav' + i} className={styles.tickWrapper}>
        ...
        <div className={styles.cta}>
          <div className={styles.hero}>
            <h1>
              COMECE JÁ
              <br />
              <span className={styles.article}>O</span>{' '}
              <em className={styles.business}>SEU COMÉRCIO</em>
            </h1>
            <Button
              to={'/#contato'}
              onClick={this.scrollToLink}
              className={styles.ctaBtn}
            >
              MONTE SUA LOJA
            </Button>
            {/* <ScrollToHash to={'/#contato'}>Any</ScrollToHash> */}
          </div>
        </div>
